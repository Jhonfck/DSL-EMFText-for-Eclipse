...
/**
 * The OfficeTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * OfficeTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class OfficeTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.emftext.language.office.resource.office.mopp.OfficeTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.emftext.language.office.resource.office.mopp.OfficeTaskItem> foundItems = new java.util.ArrayList<org.emftext.language.office.resource.office.mopp.OfficeTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.office.resource.office.mopp.OfficeTaskItem(keyword, message, localLine, offset, end));

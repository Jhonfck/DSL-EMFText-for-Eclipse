...
public class OfficeDefaultNameProvider implements org.emftext.language.office.resource.office.IOfficeNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.office.resource.office.util.OfficeEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
